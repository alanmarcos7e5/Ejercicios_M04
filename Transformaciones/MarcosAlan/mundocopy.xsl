...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Substituye XXXXXX por el path que corresponda y 
	...
	<xsl:template match="mundo">
		<xsl:for-each select="continente">
			...
			<table style="{concat('border: 2px solid ',nombre/@color)}">
				...
				<xsl:apply-templates select="paises" /> <!--llamo al template, dentro del for each-->
		...
		<xsl:for-each select="pais">
			...
			<tr>
				<th style="{concat('border: 2px solid ',$contColor)}">
					<img src="{concat('img/',foto)}" alt="banderita"/>
				</th>
				<th style="{concat('border: 2px solid ',$contColor)}">
					<xsl:value-of select="nombre"></xsl:value-of>
				</th>
				<th style="{concat('border: 2px solid ',$contColor)}"> <!--se va este style, nose como dar dos styles-->
